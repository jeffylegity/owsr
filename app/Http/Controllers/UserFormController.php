...
class UserFormController extends Controller{
   ...
   public function userSubmitReqForm(Request $request){
      ...
      $route_to_supervisor = $request->input('route_to_supervisor');
      $route_to_manager    = $request->input('route_to_manager');
      $request_status      = 'pending to supervisor';
      $date_requested      = now();
      $req_limiter         = DB::table('requests')->select('*')->where(['requestor'=>$this->session_id,'request_status'=>'pending to supervisor'])->count();
      
      if ($request_to == 'EQUIPMENT ENGINEERING') {

         $this->redirect_to = redirect()->route('user.ee_req_form');

      } elseif ($request_to == 'PLANT MAINTENANCE') {

         $this->redirect_to = redirect()->route('user.pm_req_form');

      } elseif ($request_to == 'PROCESS ENGINEERING') {

         $this->redirect_to = redirect()->route('user.pe_req_form');
